...
import cucerdariancatalin.ux.R
import cucerdariancatalin.ux.main.MainActivity
import cucerdariancatalin.ux.main.animationPlaybackSpeed
import cucerdariancatalin.ux.utils.*
import cucerdariancatalin.ux.views.NoScrollRecyclerView
...
class FiltersLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    private fun openFilterSheet(isOpening: Boolean) {
        ...
        val revealAnimator = getValueAnimator(
            isOpening, revealAnimationDuration, AccelerateDecelerateInterpolator()
        ) { progress ->
        ...
        val settleAnimator = getValueAnimator(isOpening, settleAnimDuration,
            if (isOpening) DecelerateInterpolator() as TimeInterpolator else AccelerateInterpolator()
        ) { progress ->
    ...
    private fun onFilterApplied() {
        ...
        val fadeOutAnimator = getValueAnimator(
            true, collapseAnimDuration * 30 / 100, AccelerateInterpolator()
        ) { progress ->
    ...
    private fun getCloseIconAnimator() = getValueAnimator(
        true, loadingDuration, DecelerateInterpolator()
    ) { progress ->
